...
function CanvasState(canvas) {
  ...
  // **** First some setup! ****
  this.canvas = canvas;
  this.highlightEdges = new Set(); //canonical edge keys to highlight in colour
  this.highlightColor = '#cc0000'; //red by default
...
CanvasState.prototype.draw = function() {
  ...
  // draw edges; differing edges in highlightColor, the rest in dark blue
  for (let i = 0; i < height; i++) {
    for (let j = 0; j < width; j++) {
      let moves = tour[i][j];
      for (let k = 0; k < 2; k++) {
        ...
        let key = canonicalEdgeKey(i, j, i2, j2);
        let isDiff = this.highlightEdges.has(key);
        ctx.beginPath();
        ...
        ctx.strokeStyle = isDiff ? this.highlightColor : '#1a4480';
        ctx.lineWidth   = isDiff ? 2.4              : 0.9;
        ctx.stroke();
      }
    }
  }
...
//=== Single canvas + checkbox to toggle BJMOW vs patched ===
//=== Edges that differ between the two are drawn in red (patched) or green (orig). ===
var CS = new CanvasState(document.getElementById('canvas_main'));
var lastTourOrig = null, lastTourPatched = null;
var lastOnlyPatched = new Set();   //edges in patched tour only
var lastOnlyOrig    = new Set();   //edges in original tour only
init();
...
function refresh(n) {
  ...
  lastTourOrig    = to;
  lastTourPatched = tp;
  let eo = tourEdges(to), ep = tourEdges(tp);
  lastOnlyPatched = new Set([...ep].filter(x => !eo.has(x)));
  lastOnlyOrig    = new Set([...eo].filter(x => !ep.has(x)));
  applyMode();
}
...
function applyMode() {
  ...
  if (usePatch) {
    CS.tour = lastTourPatched;
    CS.highlightEdges = lastOnlyPatched;
    CS.highlightColor = '#cc0000'; //red = SequenceP_40 internal edges
  } else {
    CS.tour = lastTourOrig;
    CS.highlightEdges = lastOnlyOrig;
    CS.highlightColor = '#1aa01a'; //green = Sequence1 edges that the patch replaces
  }
  CS.valid = false;
  let xo = numCrossings(lastTourOrig);
  let xp = numCrossings(lastTourPatched);
  let cur = usePatch ? xp : xo;
  let saving = xo - xp;
  setLabelById('crossingcount',
    'Crossings: ' + cur +
    (usePatch
       ? '  \u2014 patched tour (saving ' + saving + ' over BJMOW = ' + xo + ')'
       : '  \u2014 BJMOW tour (' + saving + ' more than patched = ' + xp + ')'));
...
//canonical edge key for an unordered pair of cells.
function canonicalEdgeKey(i1, j1, i2, j2) {
...
function tourEdges(tour) {
  let edges = new Set();
  let h = tour.length, w = tour[0].length;
  for (let i = 0; i < h; i++) {
    for (let j = 0; j < w; j++) {
      let moves = tour[i][j];
      for (let k = 0; k < 2; k++) {
        let m = parseInt(moves[k]);
        edges.add(canonicalEdgeKey(i, j, i + movesI[m], j + movesJ[m]));
      }
    }
  }
  return edges;
